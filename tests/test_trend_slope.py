...
def test_regime_misalignment_downgrades_and_hard_block_drops():
    ...
    assert any(r.name == "trend_regime" for r in short.entry_downgrades)
...
def test_regime_slope_misalignment_downgrades():
    ...
    short = [c for c in out if c.direction is Direction.SHORT][0]
    assert short.is_strong is False
    assert any(r.name == "slope_regime" for r in short.entry_downgrades)
...
def test_aligned_candidate_untouched_and_missing_passes_through():
    ...
    assert all(r.name not in {"trend_regime", "slope_regime"} for r in c1.entry_downgrades)
    ...
    assert all(r.name not in {"trend_regime", "slope_regime"} for r in c2.fired_rules)
    assert all(r.name not in {"trend_regime", "slope_regime"} for r in c2.entry_downgrades)
